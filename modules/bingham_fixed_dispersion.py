...
class BinghamFixedDispersionPositionLoss(object):
    def __init__(self, bd_z):
        self.bingham_fixed_dispersion_loss = BinghamFixedDispersionLoss(bd_z)

    def __call__(self, target, output, device="cuda:0", positions=False):
        """
        Combined pose and orientation loss for visual odometry. Uses
        bingham_fixed_dispersion_position_batch for the pose loss and
        assumes independent Gaussians for the position loss.

        Requires NN to have 16+6=22 outputs

        Parameters:
           target: Target values at which the likelihood is evaluated
                  of shape (N, 3+4=7)
           output: Output values from which M is extracted, shape
                  (N, 6+16=22)
        Returns:
           log likelihood loss: sum of the log of the pdf of each of the
                               target samples
        """

        target_orient = target[..., 3:].reshape(-1, 4)
        output_orient = output[..., 6:]

        _, mean_log_likelihood = self.bingham_fixed_dispersion_loss(
                target_orient, output_orient)
        loss = -1 * mean_log_likelihood

        if positions:
            target_pose = target[..., :3]
            output_pose = output[..., :6]

            mu, logsigma = torch.split(output_pose, 3, dim=-1)
            loss_pos = -batched_logprob(target_pose, mu, torch.exp(logsigma))
            loss += loss_pos
